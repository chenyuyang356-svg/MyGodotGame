...
void AudioManager::_bind_methods() {
    ClassDB::bind_method(D_METHOD("play_bgm", "path", "volume", "fade_duration"), &AudioManager::play_bgm, DEFVAL(0.0f), DEFVAL(1.5f));
    ClassDB::bind_method(D_METHOD("stop_bgm", "fade_duration"), &AudioManager::stop_bgm, DEFVAL(1.5f));
    ClassDB::bind_method(D_METHOD("play_sfx_3d", "path", "pos", "volume", "pitch_range"), &AudioManager::play_sfx_3d, DEFVAL(0.0f), DEFVAL(0.1f));
    ClassDB::bind_method(D_METHOD("play_ui_sfx", "path", "volume"), &AudioManager::play_ui_sfx, DEFVAL(0.0f));
...
AudioManager::AudioManager() {
    ...
    for (int i = 0; i < pool_size; ++i) {
        ...
        player->set_max_distance(1500.0f);
...
Ref<AudioStream> AudioManager::_get_audio_stream(const String& p_path) {
    if (sound_cache.has(p_path)) {
        return sound_cache[p_path];
    }
    ...
    if (stream.is_valid()) {
        ...
        UtilityFunctions::print("[AudioManager] Error: Could not load sound: ", p_path);
    }
    return stream;
}
...
void AudioManager::play_sfx_3d(String p_path, Vector3 p_pos, float p_volume, float p_pitch_range) {
    ...
    // 从池中选取播放器
    AudioStreamPlayer3D* player = sfx_pool_3d[next_pool_idx];

    // 如果当前播放器正在忙，强制停止它（对于短促音效这是可以接受的）
    if (player->is_playing()) {
        player->stop();
    }

    // 设置位置（由于你是 2D 逻辑，强制 Y 轴对齐到 0 以获得一致的左右平衡感）
    ...
    // 随机音高：防止连续开火等音效听起来由于完全一致而产生“机器感”
    ...
    // 循环指向池中下一个
    next_pool_idx = (next_pool_idx + 1) % pool_size;
}
