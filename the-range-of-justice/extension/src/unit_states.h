...
namespace godot {
    ...
    public:
        ...
        float attack_interval = 1.0f; // 对应“攻击频率”，通常存间隔更容易计算
        float splash_radius = 0.0f;   // 0 代表单体
        AttackType attack_type = ATTACK_PHYSICAL;
        float projectile_speed = 0.0f; // 0 代表瞬发(Hitscan)
        TargetPriority target_priority = PRIORITY_CLOSEST;

        // --- 移动与物理 ---
        float move_speed = 200.0f;
        float turn_speed = 5.0f;      // 弧度/秒 或 角度/秒
        float collision_radius = 10.0f;
        MoveType move_type = MOVE_GROUND;

        // --- 感知 ---
        float sight_range = 300.0f;
        float aggro_range = 250.0f; // 警戒范围
    ...
    public:
        ...
        void set_unit_tags(int p_value) { unit_tags = p_value; }
        int get_unit_tags() const { return unit_tags; }
    };

}

VARIANT_ENUM_CAST(UnitStats::ArmorType);
VARIANT_ENUM_CAST(UnitStats::AttackType);
VARIANT_ENUM_CAST(UnitStats::MoveType);
VARIANT_ENUM_CAST(UnitStats::TargetPriority);
VARIANT_BITFIELD_CAST(UnitStats::UnitTag);