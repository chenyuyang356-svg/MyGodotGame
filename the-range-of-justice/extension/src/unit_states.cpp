...
void UnitStats::_bind_methods() {
    // ===========================
    // 1. 绑定枚举常量 (使得 GDScript 可以访问 UnitStats.ARMOR_HEAVY)
    // ===========================
    ...
    // ===========================
    // 2. 绑定属性 (Add Groups and Properties)
    // ===========================

    // --- Group: Survival (生存属性) ---
    ADD_GROUP("Survival", "health_"); // 前缀用于折叠，这里写的不完全匹配也没事，主要看第一个参数
    ...
    // Unit Tags (Bitmask)
    // 使用 PROPERTY_HINT_FLAGS 让编辑器显示为复选框
